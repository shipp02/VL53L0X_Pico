...
// Most of the functionality of this library is based on the VL53L0X library
// from Pololu which is based on the VL53L0X API provided by ST (STSW-IMG005),
// and some of the explanatory comments are quoted or paraphrased from the API
// source code, API user manual (UM2039), and the VL53L0X datasheet.
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg) {
    ...
    if (DEBUG_READ) {
        printf("r4 %i : %i %i %i %i\n", regBuf, value[0], value[1], value[2],
               value[3]);
    }
    return (uint32_t)value[0] * 16777216 + value[1] * 65536 + value[2] * 256 +
           value[3];
}
